...
def function():
    ...
    AB_im = struct.unpack(helper, fpga.read('AB_im', channels*8, 0)) # ojo aca debiese ser q en vez de Q (osea q el signo si me importa...)
    print('A = '+str( 10*np.log10(struct.unpack('>1Q', fpga.read('A2', 8 ,6068*8 )))))
  #  print('crudo= '+str(A2[6068]))
    log_a = 10*np.log10(A2)
    log_b = 10*np.log10(B2)
    ang = np.rad2deg(np.arctan2(AB_im, AB_re))
    #print('angle = '+str(ang[5993]))
