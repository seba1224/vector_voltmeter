...
class Get_data():
    ...
    def obtain_data(self):
        ...
        ##MODIFY THIS
        helper = helper = '>'+str(self.index+2*self.index_offset)+'Q'
        while(enabler):
            A2 = struct.unpack(helper, fpga.read('A2', (self.index+2*self.index_offset)*8, (self.index-self.index_offset)*8))
            self.amp_a2[0] = np.mean(A2)
            self.amp_a2 = np.roll(self.amp_a2, -1)
            B2 =  struct.unpack(helper, fpga.read('A2', (self.index+2*self.index_offset)*8, (self.index-self.index_offset)*8))
            self.amp_a2[0] = np.mean(A2)
            self.amp_a2 = np.roll(self.amp_a2, -1)
            AB_re = struct.unpack(helper, fpga.read('A2', (self.index+2*self.index_offset)*8, (self.index-self.index_offset)*8))
            self.ab_re[0] = np.mean(AB_re)
            self.ab_re = np.roll(self.ab_re, -1)
            AB_im = struct.unpack(helper, fpga.read('A2', (self.index+2*self.index_offset)*8, (self.index-self.index_offset)*8))
            self.ab_im[0] = np.mean(AB_im)
            self.ab_im = np.roll(self.ab_im, -1) 

            log_a = 10*np.log10(np.mean(self.amp_a2))
            log_b = 10*np.log10(np.mean(self.amp_b2))             
